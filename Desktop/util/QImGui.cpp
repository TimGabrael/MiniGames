...
#undef OFFSETOF
    ...
    void* initialize(QWidget* window, bool defaultRender) {
        if (defaultRender) {
            auto* wrapper = new QWidgetWindowWrapper(window, ImGuiRenderer::instance());
            ImGuiRenderer::instance()->initialize(wrapper);
            return reinterpret_cast<void*>(dynamic_cast<QWindowWrapper*>(wrapper));
        }
        else {
    ...
    void* initialize(QWindow* window, bool defaultRender) {
        if (defaultRender) {
