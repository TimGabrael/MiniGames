...
void Connection::SendData(PacketID id, uint32_t group, uint16_t additionalFlags, uint16_t clientID, size_t size, const void* data)
{
	uint8_t* packData = new uint8_t[size];
	memcpy(packData, data, size);
	PacketHeader pack(id, group, additionalFlags, clientID, size);
...
void Connection::SendData(PacketID id, uint32_t group, uint16_t additionalFlags, uint16_t clientID, const std::string& str)
{
	SendData(id, group, additionalFlags, clientID, str.size(), str.data());
}
...
void TCPSocket::SendData(PacketID id, uint32_t group, uint16_t additionalFlags, uint16_t clientID, size_t size, const void* data)
{
	serverConn.SendData(id, group, additionalFlags, clientID, size, data);
}
void TCPSocket::SendData(PacketID id, uint32_t group, uint16_t additionalFlags, uint16_t clientID, const std::string& str)
{
	serverConn.SendData(id, group, additionalFlags, clientID, str);
}
